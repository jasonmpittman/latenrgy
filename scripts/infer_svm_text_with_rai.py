...
def load_model():
    ...
    model_path = os.path.join(MODEL_DIR, "svm_text_model.pkl")
...
def main():
    ...
    tracker = EmissionsTracker(allow_multiple_runs=True, output_file="emissions/emissions_svm_text_inference_rai.csv")
    tracker.start()
    ...
    # Run inference
    y_pred = run_inference(svm_model, X_inference)
    ...
    explain_with_precomputed_shap(svm_model, X_inference, SHAP_DIR, model_type="SVM", dataset="text")
    ...
    results_path = os.path.join(RESULTS_DIR, "svm_text_inference_with_rai_results.txt")
    with open(results_path, "w") as f:
        f.write("--- SVM Text Inference with RAI Results ---\n")
