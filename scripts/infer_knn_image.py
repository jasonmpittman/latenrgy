...
RESULTS_PATH = "results/knn_image_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/image/knn"
...
def save_bar_chart(metrics):
    ...
    plt.title("k-NN Performance Metrics (Inference - Images)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    # Load trained model
    print("Loading trained k-NN model...")
    model = joblib.load(MODEL_PATH)

    # Initialize energy tracker
    tracker = EmissionsTracker(output_file="emissions/emissions_image_knn_inference.csv", allow_multiple_runs=True)
    tracker.start()

    # Measure inference latency
    print("Running inference...")
    start_time = time.time()
    y_pred = model.predict(X.reshape(X.shape[0], -1))  # Flatten images for k-NN input
    ...
    os.makedirs(VISUALIZATIONS_DIR, exist_ok=True)
    plt.title("k-NN Confusion Matrix (Inference - Images)")
