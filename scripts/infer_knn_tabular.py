...
RESULTS_PATH = "results/knn_tabular_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/tabular/knn"
...
def save_bar_chart(metrics):
    ...
    plt.title("k-NN Performance Metrics (Inference - Tabular)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    tracker = EmissionsTracker(output_file="emissions/emissions_tabular_knn_inference.csv", allow_multiple_runs=True)
    tracker.start()
    ...
    os.makedirs(VISUALIZATIONS_DIR, exist_ok=True)
    plt.title("k-NN Confusion Matrix (Inference - Tabular)")
