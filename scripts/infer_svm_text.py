...
RESULTS_PATH = "results/svm_text_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/text/svm"
...
def save_bar_chart(metrics):
    ...
    plt.title("SVM Performance Metrics (Inference - Text)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    tracker = EmissionsTracker(allow_multiple_runs=True, output_file="emissions/emissions_svm_text_inference.csv")
    tracker.start()

    # Perform inference
    print("Running inference...")
    start_time = time.time()
    y_pred = svm.predict(X_inference)
    ...
    if y_true is not None:
        ...
        plt.title("SVM Confusion Matrix (Inference - Text)")
