...
LIME_OUTPUT_DIR = os.path.join(RESULTS_DIR, "lime_rf_interpretability")
...
def load_model():
    ...
    print(f"Loading Random Forest model from: {MODELS_DIR}")
    model = joblib.load(os.path.join(MODELS_DIR, "rf_text_model.pkl"))
    return model
...
def evaluate_model(model, X_data, y_data):
    ...
    # Energy tracker
    tracker = EmissionsTracker(allow_multiple_runs=True, output_file=os.path.join(EMISSIONS_DIR, "emissions_rf_text_with_interpretability.csv"))
    tracker.start()
    ...
    # Save results
    results_path = os.path.join(RESULTS_DIR, "rf_text_with_interpretability_results.txt")
...
def main():
    ...
    evaluate_model(rf_model, X_data, y_data)
