...
RESULTS_PATH = "results/knn_text_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/text/knn"
...
def save_bar_chart(metrics):
    ...
    plt.title("k-NN Performance Metrics (Inference - Text)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    tracker = EmissionsTracker(allow_multiple_runs=True, output_file="emissions/emissions_knn_text_inference.csv")
    tracker.start()

    # Perform inference
    print("Running inference...")
    start_time = time.time()
    y_pred = knn.predict(X_inference)
    ...
    if y_true is not None:
        ...
        plt.title("k-NN Confusion Matrix (Inference - Text)")
