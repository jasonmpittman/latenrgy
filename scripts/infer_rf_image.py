...
RESULTS_PATH = "results/rf_image_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/image/rf"
...
def save_bar_chart(metrics):
    ...
    plt.title("Random Forest Performance Metrics (Inference - Images)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    tracker = EmissionsTracker(output_file="emissions/emissions_image_rf_inference.csv", allow_multiple_runs=True)
    tracker.start()

    # Measure inference latency
    print("Running inference...")
    start_time = time.time()
    y_pred = model.predict(X.reshape(X.shape[0], -1))  # Flatten images for RF input
    ...
    os.makedirs(VISUALIZATIONS_DIR, exist_ok=True)
    plt.title("Random Forest Confusion Matrix (Inference - Images)")
