...
RESULTS_PATH = "results/svm_tabular_inference_results.json"
VISUALIZATIONS_DIR = "visualizations/tabular/svm"
...
def save_bar_chart(metrics):
    ...
    plt.title("SVM Performance Metrics (Inference - Tabular)")
    plt.xlabel("Metric")
    plt.ylabel("Value")
    plt.ylim(0, 1)
...
def main():
    ...
    tracker = EmissionsTracker(output_file="emissions/emissions_tabular_svm_inference.csv", allow_multiple_runs=True)
    tracker.start()
    ...
    os.makedirs(VISUALIZATIONS_DIR, exist_ok=True)
    plt.title("SVM Confusion Matrix (Inference - Tabular)")
