...
def load_model():
    ...
    model_path = os.path.join(MODEL_DIR, "rf_text_model.pkl")
...
def main():
    ...
    tracker = EmissionsTracker(allow_multiple_runs=True, output_file="emissions/emissions_rf_text_inference_rai.csv")
    tracker.start()
    ...
    # Run inference
    y_pred = run_inference(rf_model, X_inference)
    ...
    explain_with_precomputed_shap(rf_model, X_inference, SHAP_DIR, model_type="RandomForest", dataset="text")
    ...
    results_path = os.path.join(RESULTS_DIR, "rf_text_inference_with_rai_results.txt")
    with open(results_path, "w") as f:
        f.write("--- Random Forest Text Inference with RAI Results ---\n")
